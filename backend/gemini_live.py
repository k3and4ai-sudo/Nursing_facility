...
class GeminiLiveSession:
            ...
            try:
                ...
                if self.history:
                    ...
                    for h in self.history[-8:]:
                        ...
                        raw_msg = h.get("message", "").strip()
                        # Strip internal robotic preambles completely from history
                        msg = re.sub(r"みまもりさん[、へ]?[^。.\n]+[。.・\n]?", "", raw_msg).strip()
                        msg = re.sub(r"^[「」\s]+", "", msg).strip()
                        # Ignore system command leftovers like schedule closure
                        if any(k in msg for k in ["予定カードを閉じました", "予定カードの表示を終了", "予定を閉じて", "予程を閉じて", "予定消して"]):
                            continue
                        if not msg or msg == last_msg_text:
                            continue
                        # Deduplicate repeated greeting turns
                        if msg.startswith("こんにちは") and last_msg_text.startswith("こんにちは"):
                            continue
                ...
                if self.schedules:
                    ...
                    today_schedules_text = (
                        ...
                        "\n".join(sched_lines) +
                        "\n※利用者様から「今日の予定は何だっけ？」「午後は何がある？」「お風呂は何時？」などと聞かれたら、現在時刻を考慮し、終了した予定には「〜はもう終了しましたよ」とお伝えし、これからの予定を優しく分かりやすく教えてあげてください。上記にない架空の予定（レクリエーション等）を勝手に創作することは厳禁です。\n"
                    )
                    schedule_startup_instruction = (
                        f"★【起動時（最初の挨拶時）のみ】：\n"
                        f"会話開始（起動）時の最初の挨拶の時のみ、第一声で正確に：\n"
                        f"「みまもりさん予定カードの表示をお願いします。」\n"
                        f"と発言し、続けて「{nickname}、こんにちは！本日の予定を表示しましたよ」とお伝えください。挨拶後は自分から予定を長々と語らず、利用者の言葉を待ってください。\n\n"
                    )
                else:
                    today_schedules_text = (
                        f"\n【{nickname}の今日のご予定（{now_dt.strftime('%Y年%m月%d日')} 現在時刻: {current_hhmm}）】\n"
                        f"・本日は予定は登録されていません（本日の予定は【なにもありません】）。\n"
                        f"※【最重要・厳重警告：架空の予定の捏造・創作は絶対に厳禁！】\n"
                        f"本日は予定が一切ありません！「午後にレクリエーションがある」「午後に散歩がある」等の架空の予定を捏造・でっち上げることは【絶対に禁止】です！\n"
                        f"利用者様から予定を聞かれたり話題にする場合は、必ず「本日は特に予定は入っていませんよ。お部屋でゆっくりお過ごしくださいね」と正確にお答えください。\n"
                    )
                    schedule_startup_instruction = (
                        f"★【起動時（最初の挨拶時）のみ】：\n"
                        f"本日は予定が一切登録されていません。会話開始（起動）時の最初の挨拶では、\n"
                        f"「{nickname}、こんにちは！本日は特に予定は入っていませんので、ゆっくり過ごしてくださいね」\n"
                        f"とだけ温かく挨拶し、利用者の言葉を待ってください。\n"
                        f"※予定がないため、「みまもりさん予定カードの表示をお願いします」や「午後にレクリエーションがある」等の架空の案内は【絶対に言わないでください】。\n\n"
                    )
                    ...
                    "setup": {
                        ...
                        "systemInstruction": {
                            "parts": [
                                {
                                    "text": (
                                        f"あなたの名前は「ジェミナイ（Gemini）」です。介護施設の高齢者ケアに特化したGemini Live会話AIアシスタントです。"
                                        ...
                                        f"【重要指示2-3：本日の予定カードについて（起動時のみ、起動後は依頼不要）】\n"
                                        f"{schedule_startup_instruction}"
                                        f"★【厳重禁止ルール（予定の話題への固執・架空の予定創作・謝罪の厳禁）】：\n"
                                        f"・会話開始後（起動後）は、「みまもりさん予定カードの表示をお願いします」や「みまもりさん予定カードの表示を終了してください」を【絶対に言ってはいけません】。\n"
                                        f"・自分から「午後に楽しいレクリエーションが〜」などの架空の予定を捏造したり、相手の発言を無視して予定の話を繰り返すことは【厳重に禁止】です！\n"
                                        ...
                                        f"・ただし、一度挨拶を交わした後は、同じ会話の中で何度も「こんにちは」を繰り返してはいけません！相手が別のこと（質問、相槌、日常の出来事など）を話した時は、挨拶を蒸し返さず、必ずその発言内容に直接答えてください。\n"
                                        ...
                                        f"【重要指示5：利用者様が希望された場合のデジタル絵手紙の下絵作成・確認・修正】\n"
    ...
    def _handle_text_chunk(self, text_val: str):
        ...
        is_to_hide_etegami = any(p in raw_norm for p in hide_patterns) or "hide etegami" in text_lower or "close etegami" in text_lower
        if not is_to_hide_etegami and ("絵" in raw_norm or "手紙" in raw_norm) and any(k in raw_norm for k in ["閉じて", "とじて", "閉じる", "とじる", "消して", "けして"]):
            if not any(k in raw_norm for k in ["描きたい", "かきたい", "書きたい", "出して", "表示"]):
                is_to_hide_etegami = True
